...
const allowedHost = (host) =>
  STORES.some((domain) => host === domain || host.endsWith("." + domain));
...
function productsFromJsonLd(html) {
  ...
  function walk(node) {
    ...
    if (types.some((type) => /(?:^|\/)product$/.test(type))) {
      products.push(node);
    }
...
function cleanImages(raw, base) {
  ...
  return [
    ...new Set(list.map((value) => safeImage(value, base)).filter(Boolean)),
  ].slice(0, 5);
}
...
function fallbackTitle(url) {
  ...
  if (url.hostname.endsWith("myntra.com") && parts.length >= 3) {
    slug = parts[2];
  }
...
async function directPreview(productUrl) {
  ...
  if (maintenancePage(html, pageTitle)) {
    throw new Error("Store returned a maintenance or access-restricted page");
  }

  const title = decode(
    product.name || pageTitle || fallbackTitle(productUrl),
  ).slice(0, 120);
  ...
  const currency =
    ...
    first(meta, "product:price:currency", "og:price:currency") ||
    "INR";

  const price = formatPrice(
    offers.price ??
      offers.lowPrice ??
      first(meta, "product:price:amount", "og:price:amount"),
    currency,
  );

  const description = decode(
    product.description || first(meta, "description", "og:description"),
  ).slice(0, 300);

  if (!product.name && !images.length && !price) {
...
function searchTerms(url) {
  ...
  const raw =
    url.hostname.endsWith("myntra.com") && parts.length >= 3
      ? parts[2]
      : parts.at(-1) === "buy"
        ? parts.at(-3)
        : parts.at(-1);
...
function rankImage(result, terms, originalUrl) {
  ...
  const text = ((result.title || "") + " " + (result.link || "")).toLowerCase();
  ...
  if (score < Math.min(6, keywords.length * 2)) {
    return -1;
  }

  if (terms.id && text.includes(terms.id)) {
    score += 12;
  }

  if ((result.link || "").includes(originalUrl.hostname)) {
    score += 8;
  }

  if (
    /newbalance\.(com|in)|adidas\.(com|co\.in)|nike\.com/i.test(
      result.link || "",
    )
  ) {
    score += 3;
...
async function searchPublicImages(url) {
  ...
  const query = terms.name + (terms.id ? " " + terms.id : "");
  ...
  if (!response.ok) {
    throw new Error("Image search HTTP " + response.status);
  }
  ...
  const sorted = (body.images_results || [])
    .map((result) => ({
      result,
      score: rankImage(result, terms, url),
    }))
  ...
  const candidates = sorted
    .map(({ result, score }) => ({
      image: safeImage(result.original || result.thumbnail),

      thumbnail: safeImage(result.thumbnail || result.original),

      source: safeImage(result.link),

      title: decode(result.title || "").slice(0, 120),

      score,
    }))
    .filter((candidate) => candidate.image || candidate.thumbnail);

  // Prefer candidates containing the exact product ID.
  const exact = candidates.filter(
    (candidate) =>
      terms.id && (candidate.title + " " + candidate.source).includes(terms.id),
  );

  // Other matches remain suggestions and should be verified.
  const suggested = exact.length ? exact : candidates.slice(0, 1);

  return {
    images: suggested
      .slice(0, 5)
      .map((candidate) => candidate.thumbnail || candidate.image),
...
exports.handler = async (event) => {
  ...
  if (
    ...
    (productUrl.port && productUrl.port !== "443") ||
    productUrl.username ||
    productUrl.password ||
  ...
  try {
    ...
    console.warn("Retailer preview unavailable:", retailerError);
  }
  ...
  try {
    const results = await searchPublicImages(productUrl);
    ...
    return send(200, {
      ...
      previewAvailable: product.previewAvailable || results.images.length > 0,

      retailerError,
    });
  } catch (error) {
    console.warn("Image search unavailable:", error.message);
    ...
    return send(200, {
      ...
      searchStatus:
        "Image search unavailable: " + String(error.message).slice(0, 120),
